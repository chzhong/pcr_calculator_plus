...
# from pip
from sympy import Eq, parse_expr, solve, symbols
...
async def send_help(bot: HoshinoBot, ev: CQEvent):
    await bot.send(
        ev, MessageSegment.image(f"file:///{gs_current_dir / 'data' / 'help.png'}")
    )
...
def try_prase(raw_packs: List[Pack], pack_types: str) -> Union[list, None]:
    ...
    assert len(raw_packs) == len(pack_types), (
        f"Internal Error: len(packs)[{len(raw_packs)}] != len(pack_types)[{len(pack_types)}]\npack_types={pack_types}"
    )
    outp = []
...
def format_damage_for_report(damage: Union[int, None]) -> str:
    if damage is None:
        return "无解"
    if damage < 100_0000:
        return str(damage)
    if damage < 1_0000_0000:
        return f"{math.ceil(damage / 1_0000)}w"
    yi = math.ceil(damage / 1_0000_0000 * 100) / 100
    return f"{yi:.2f}e"
...
def solve_equation(e, b, d, t=0) -> Union[int, None]:
    ...
    eqn = Eq(_e, 110 - (90 - _t) * _b / _d).subs(
        {_e: e_x.rhs, _b: b_x.rhs, _d: d_x.rhs, _t: t_x.rhs}
    )
...
def B(raw_packs: List[Pack]) -> Output:
    ...
    def _(b: PackB) -> Output:
        ...
        for i in range(1, 4 + 1):
            output.append(
                f"{i}刀 \t {format_damage_for_report(solve_equation(89 + delta, f'{b.D}-{i - 1}*x', 'x'))}"
            )
    ...
    def _(b: PackB, dt: PackDT) -> Output:
        ...
        if dt.T is None:  # B_D
            if b.D > dt.D:  # cal 700 400
                ...
                if 另一刀后出 >= b.D:
                    output.append(
                        f"若{dt.D // 10000}w先出，后出刀需{format_damage_for_report(另一刀后出)}伤害（高于boss血量）才能满补"
                    )
                else:
                    output.append(
                        f"若{dt.D // 10000}w先出，后出刀需{format_damage_for_report(另一刀后出)}伤害可满补"
                    )
                另一刀先出 = solve_equation(89 + delta, f"{b.D}-x", dt.D)
                output.append(
                    f"若{dt.D // 10000}w后出，先出刀需{format_damage_for_report(另一刀先出)}伤害可满补"
                )
                合刀补偿 = 两人合刀补偿(b.D, dt.D)
                if 合刀补偿 is not None:
                    output.append(f"若两人合刀，则补偿{合刀补偿}s")
            else:  # cal 400 700
                e = min(solve_equation("x", b.D, dt.D), 90)
                output.append(f"补偿{e}s")
                if e < 90:
                    output.append(
                        f"垫入{format_damage_for_report(solve_equation(89 + delta, f'{b.D}-x', dt.D))}伤害可满补"
                    )
        else:  # B_T B_DT
            if dt.D is None:  # B_T
                dt.D = b.D
            ...
            if b.D > dt.D:
                return Output(
                    OutputFlag.Error,
                    f"参数不合法：boss血量[{b.D}]高于尾刀伤害[{dt.D}余{dt.T}s]",
                )
            ...
            if 返还 < 90:
                if dt.D == b.D:
                    ...
                    if 返还 >= 一穿二:
                        ...
                        output.append(
                            f"需返还{一穿二}s以一穿二，还需垫入{format_damage_for_report(solve_equation(使用 + delta, f'{b.D}-x', dt.D, dt.T))}伤害"
                        )
                output.append(
                    f"垫入{format_damage_for_report(solve_equation(89 + delta, f'{b.D}-x', dt.D, dt.T))}伤害可满补"
                )
...
def B_E(raw_packs: List[Pack]) -> Output:
    ...
    def _(b: PackB, e: PackE) -> Output:
        ...
        for i in range(1, 3 + 1):
            output.append(
                f"{i}刀 \t {format_damage_for_report(solve_equation(e.T - 1 + delta, f'{b.D}-{i - 1}*x', 'x'))}"
            )
        return Output(OutputFlag.Succeed, "\n".join(output))
    ...
    def _(b: PackB, dt1: PackDT, dt2: PackDT) -> Output:
        ...
        if dt1.T is None and dt2.T is None:  # B_D_D
            ...
            if ds >= b.D:
                ...
                output.append(f"剩余{format_damage_for_report(b.D - ds - db)}血")
            else:
                ...
                if db < b.D:
                    if ds先出 == db先出:
                        ...
                        output.append(
                            f"若[{format_damage_for_report(ds)}]先出，[{format_damage_for_report(db)}]后出，补偿{ds先出}s"
                        )
                        output.append(
                            f"若[{format_damage_for_report(db)}]先出，[{format_damage_for_report(ds)}]后出，补偿{db先出}s"
                        )
                else:
                    output.append(
                        f"若[{format_damage_for_report(ds)}]先出，[{format_damage_for_report(db)}]后出，补偿{ds先出}s"
                    ...
                    output.append(
                        f"若[{format_damage_for_report(db)}]直出，不出[{format_damage_for_report(ds)}]，补偿{db先出}s"
                    )
            ...
            if d.D >= b.D:
                ...
                if db直出 == 90:
                    output.append(
                        f"[{format_damage_for_report(dt.D)}+{dt.T}s]直出已可满补，[{format_damage_for_report(d.D)}]不用出"
                    )
                else:
                    output.append(
                        f"若[{format_damage_for_report(d.D)}]先出，[{format_damage_for_report(dt.D)}+{dt.T}s]后出，补偿{ds先出}s"
                    )
                    output.append(
                        f"若[{format_damage_for_report(dt.D)}+{dt.T}s]直出，不出[{format_damage_for_report(d.D)}]，补偿{db直出}s"
                    )
    ...
    def _(b: PackB, dt: PackDT, e: PackE) -> Output:
        ...
        if dt.D >= b.D:
            ...
            if 返还 >= e.T:
                ...
                output.append(
                    f"{e}, 还需垫入{format_damage_for_report(solve_equation(e.T - 1 + delta, f'{b.D}-x', dt.D, dt.T or 0))}伤害"
                )
        else:
            output = [str(b), str(dt), str(e)]
            output.append(
                f"若{format_damage_for_report(dt.D)}后出，需先垫入{format_damage_for_report(solve_equation(e.T - 1 + delta, f'{b.D}-x', dt.D))}伤害"
            )
            output.append(
                f"若{format_damage_for_report(dt.D)}先出，需再接上{format_damage_for_report(solve_equation(e.T - 1 + delta, b.D - dt.D, 'x'))}伤害"
            )
